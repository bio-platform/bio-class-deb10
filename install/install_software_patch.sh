...
tmp_timestamp=$(egrep "timestamp_(remote|local)" /home/"${BIOUSER}"/HTTPS/startHTTPS.sh)
if [[ -z "$tmp_exist" ]] || [[ -z "$tmp_timestamp" ]] ;then
...
tmp_backupHTTPS=$(egrep "* 1,13 *" /etc/cron.d/backupHTTPS)
if [[ -n "$tmp_timestamp" ]];then
